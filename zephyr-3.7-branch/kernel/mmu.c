 ...
 *
 * TODO: Add optional support for copy-on-write mappings
 * to a zero page instead of allocating, in which case page frames will be
 * allocated lazily as the mappings to the zero page get touched. This will
 * avoid expensive page-ins as memory is mapped and physical RAM or backing
 * store storage will not be used if the mapped memory is unused. The cost is
 * an empty physical page of zeroes.
 */
...
void z_mem_manage_init(void)
{
	...
	/* Pin the page frames corresponding to the pinned symbols */
...
int k_mem_page_frame_evict(uintptr_t phys)
{
	...
	__ASSERT(page_frames_initialized, "%s called on 0x%lx too early",
		 __func__, phys);
	__ASSERT(k_mem_is_page_frame(phys), "0x%lx is not a page frame", phys);
	pf = k_mem_phys_to_page_frame(phys);
	__ASSERT(k_mem_page_frame_is_mapped(pf), "0x%lx is not mapped", phys);
	key = irq_lock();
	flags = arch_page_info_get(k_mem_page_frame_to_virt(pf), &phys, false);
	dirty = (flags & ARCH_DATA_PAGE_DIRTY) != 0;
	ret = page_frame_prepare_locked(pf, &dirty, false, &location);
	if (ret != 0) {
		irq_unlock(key);
...
int k_mem_page_in(void *addr, size_t size)
{
	__ASSERT(page_frames_initialized, "%s called on %p too early", __func__,
		 addr);
	k_mem_assert_virtual_region(addr, size);
	for (size_t offset = 0; offset < size; offset += CONFIG_MMU_PAGE_SIZE) {
		...
		struct k_mem_page_frame *pf;
		key = irq_lock();
		ret = arch_page_location_get(pos, &location);
		if (ret != ARCH_PAGE_LOCATION_PAGED_OUT) {
			irq_unlock(key);
			continue;
		}
		pf = free_page_frame_list_get();
		...
		arch_mem_map(pos, k_mem_page_frame_to_phys(pf), CONFIG_MMU_PAGE_SIZE,
			     K_MEM_CACHE_WB | K_MEM_PERM_RW);
		frame_mapped_set(pf, pos);
		irq_unlock(key);
		do_backing_store_page_in(location);
	}
	return 0;
}
#endif /* CONFIG_DEMAND_PAGING */
#ifdef CONFIG_USERSPACE
void z_mem_domain_init(struct k_mem_domain *domain)
{
	sys_sflist_init(&domain->partitions);
}
void z_mem_domain_partition_add(struct k_mem_domain *domain,
				struct k_mem_partition *partition)
{
	sys_sflist_append(&domain->partitions, &partition->node);
}
void z_mem_domain_partition_remove(struct k_mem_domain *domain,
				   struct k_mem_partition *partition)
{
	sys_sflist_find_and_remove(&domain->partitions, &partition->node);
}
void z_mem_domain_thread_add(struct k_thread *thread,
			     struct k_mem_domain *domain)
{
	thread->mem_domain_info.mem_domain = domain;
}
void z_mem_domain_thread_remove(struct k_thread *thread)
{
	thread->mem_domain_info.mem_domain = NULL;
}
#endif /* CONFIG_USERSPACE */
static int mmu_init(const struct device *unused)
{
	ARG_UNUSED(unused);
	z_mem_manage_init();
	return 0;
}
SYS_INIT(mmu_init, PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
//GST