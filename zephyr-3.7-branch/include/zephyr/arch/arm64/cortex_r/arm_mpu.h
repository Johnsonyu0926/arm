...
/* MPU RBAR Register attribute mask Definitions */
...
#define MPU_MAIR_ATTR_FLASH      ((NORMAL_O_WT_NT | (R_ALLOC_W_NON << 4)) | \
                                  (NORMAL_I_WT_NT | R_ALLOC_W_NON))

#define MPU_MAIR_INDEX_SRAM      2U
#define MPU_MAIR_ATTR_SRAM       ((NORMAL_O_WB_NT | (R_ALLOC_W_ALLOC << 4)) | \
                                  (NORMAL_I_WB_NT | R_ALLOC_W_ALLOC))

#define MPU_MAIR_INDEX_SRAM_NOCACHE 3U
#define MPU_MAIR_ATTR_SRAM_NOCACHE  ((NORMAL_O_NON_C | (R_NON_W_NON << 4)) | \
                                     (NORMAL_I_NON_C | R_NON_W_NON))

#define MPU_MAIR_ATTRS           ((MPU_MAIR_ATTR_DEVICE << (MPU_MAIR_INDEX_DEVICE * 8)) | \
...
struct arm_mpu_region_attr {
	uint8_t rbar : 6; /* Attributes belonging to PRBAR */
	uint8_t mair_idx : 3; /* MAIR index for attribute indirection */
};

/* Region definition data structure */
struct arm_mpu_region {
	uint64_t base; /* Region Base Address */
	uint64_t limit; /* Region limit Address */
	const char *name; /* Region Name */
	struct arm_mpu_region_attr attr; /* Region Attributes */
};

/* MPU configuration data structure */
struct arm_mpu_config {
	uint32_t num_regions; /* Number of regions */
	const struct arm_mpu_region *mpu_regions; /* Regions */
};
