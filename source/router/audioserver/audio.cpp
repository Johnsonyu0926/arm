...
loginfo(HPR_UINT32
dwLevel,
char const *pBuffer, HPR_VOIDPTR
pUserData)
{
...
void
otap_generate_sharekey(unsigned char *pDevVerificationCode, unsigned char *pDevSubSerial, unsigned char *pshare_key) {
    ...
    int ret = mbedtls_pkcs5_pbkdf2_hmac(&sha1_ctx, sharekey_dst_hex, MAX_EHOME50_KEY_LEN,
                                        (const unsigned char *) pShareKeySalt, dwShareKeySaltLen, 3,
                                        MAX_EHOME50_KEY_LEN, sharekey_sha256_dst);
...
void CALLBACK
KeyValueLoadFunc(HPR_INT32
dwHandle,
EBaseIoTKeyValueType dwKeyValueType,
unsigned char *pData, HPR_UINT32
dwLen,
...
); // todo，这两个KEYVALUE是什么作用，取什么值，在哪配
}
}
...
HPR_BOOL init_otap_reg_info(NET_EBASE_IOT_REGINFO *pRegInfo) {
    ...
    HPR_Strncpy((char *) pOTAPRegInfo->byDeviceID, g_audiocfg.business[0].deviceID.c_str(),
                sizeof(pOTAPRegInfo->byDeviceID) - 1);
    otap_generate_sharekey((unsigned char *) g_audiocfg.business[0].password.c_str(),
                           (unsigned char *) pOTAPRegInfo->byDeviceID, (unsigned char *) pOTAPRegInfo->byShareKey);
    HPR_Strncpy((char *) pOTAPRegInfo->byDeviceSerial, g_audiocfg.business[0].serial.c_str(),
                sizeof(pOTAPRegInfo->byDeviceSerial));
    HPR_Strncpy((char *) pOTAPRegInfo->bySubSerial, g_audiocfg.business[0].subSerial.c_str(),
                sizeof(pOTAPRegInfo->bySubSerial));
    HPR_Strncpy((char *) pOTAPRegInfo->byDevName, g_audiocfg.business[0].devName.c_str(),
                sizeof(pOTAPRegInfo->byDevName));
...
HPR_VOID loop_work(HPR_UINT32
dwClientType,
NET_EBASE_IOT_REGINFO struOTAPRegInfo
)
{
...
{
if (!
NET_EBASE_DeystoryClient(iHandle)
)
{
...
if (bRet)
{
cout << "success to connect to server." <<
endl;
}
else
{
