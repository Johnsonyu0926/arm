                          ...
                          char *response)
{
    ...
    if (dev->id->type != MTD_DEV_TYPE_NAND) {
        pr_err("partition '%s' is not stored on a NAND device", partname);
                         ...
                         unsigned int download_bytes, char *response)
{
    ...
    if (is_sparse_image(download_buffer)) {
        ...
        write_sparse_image(&sparse, cmd, download_buffer, download_bytes, response);
    } else {
        printf("Flashing raw image at offset 0x%llx\n", part->offset);

        ret = _fb_nand_write(mtd, part, download_buffer, part->offset, download_bytes, NULL);
