 ...
 * the bootmap.  It also expands the size of the fdt by CONFIG_SYS
 * boot_relocate_fdt() allocates a region of memory within the bootmap and
 * relocates the of_flat_tree into that region, even if the fdt is already in
	...
	*of_size = 0;
	...
	if (select || genimg_has_config(images)) {
		...
		case IMAGE_FORMAT_LEGACY:
			...
			memmove((void *)load, (void *)image_data, image_get_data_size(fdt_hdr));
		...
		case IMAGE_FORMAT_FIT:
			...
			if (fit_check_format(buf)) {
				...
				fdt_noffset = boot_get_fdt_fit(images, fdt_addr, &fit_uname_fdt,
					&fit_uname_config, arch, &load, &len);
