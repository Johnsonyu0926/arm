...
{
    return fdt_setprop_string(blob, offset, "phy-connection-type", phy_string_for_interface(phyc));
}
...
void ft_fixup_cpu(void *blob)
{
...
#if defined(CONFIG_ARMV8_SEC_FIRMWARE_SUPPORT) && defined(CONFIG_SEC_FIRMWARE_ARMV8_PSCI)
    int node;
    u32 psci_ver;
    ...
    if (psci_ver == 0xffffffff) {
        ...
        if (node >= 0) goto remove_psci_node;
...
remove_psci_node:
        if (node >= 0) fdt_del_node(blob, node);
    } else {
        return;
    }
#endif
    ...
    while (off != -FDT_ERR_NOTFOUND) {
        ...
        if (reg) {
            ...
            if (core_id == 0 || (is_core_online(core_id))) {
                val = spin_tbl_addr;
                val += id_to_core(core_id) * SPIN_TABLE_ELEM_SIZE;
    ...
    fdt_add_mem_rsv(blob, (uintptr_t)&secondary_boot_code, *boot_code_size);
#if defined(CONFIG_EFI_LOADER) && !defined(CONFIG_SPL_BUILD)
    efi_add_memory_map((uintptr_t)&secondary_boot_code, ALIGN(*boot_code_size, EFI_PAGE_SIZE) >> EFI_PAGE_SHIFT, EFI_RESERVED_MEMORY_TYPE, false);
#endif
}
#endif
...
static int _fdt_fixup_msi_node(void *blob, const char *name, int irq_0, int irq_1, int rev)
{
    int err, offset, len;
    u32 tmp[4][3];
    void *p;
    ...
    if (rev > REV1_0) {
        err = fdt_setprop_string(blob, offset, "compatible", "fsl,ls1043a-v1.1-msi");
    } else {
        err = fdt_setprop_string(blob, offset, "compatible", "fsl,ls1043a-msi");
    }
    if (err < 0) {
...
static void fdt_fixup_msi(void *blob)
{
    ...
    if (!IS_SVR_DEV(rev, SVR_DEV(SVR_LS1043A))) return;

    rev = SVR_REV(rev);

    _fdt_fixup_msi_node(blob, "/soc/msi-controller1@1571000", 116, 111, rev);
    _fdt_fixup_msi_node(blob, "/soc/msi-controller2@1572000", 126, 121, rev);
    _fdt_fixup_msi_node(blob, "/soc/msi-controller3@1573000", 160, 155, rev);
...
void fdt_fixup_remove_jr(void *blob)
{
    ...
    jr_node = fdt_node_offset_by_compatible(blob, crypto_node, "fsl,sec-v4.0-job-ring");
    ...
    while (jr_node != -FDT_ERR_NOTFOUND) {
        ...
        jr_node = fdt_node_offset_by_compatible(blob, jr_node, "fsl,sec-v4.0-job-ring");
    }
}
#endif
...
void ft_cpu_setup(void *blob, bd_t *bd)
{
    ...
    do_fixup_by_compat_u32(blob, "fsl,ns16550", "clock-frequency", CONFIG_SYS_NS16550_CLK, 1);
#endif

    do_fixup_by_path_u32(blob, "/sysclk", "clock-frequency", CONFIG_SYS_CLK_FREQ, 1);
