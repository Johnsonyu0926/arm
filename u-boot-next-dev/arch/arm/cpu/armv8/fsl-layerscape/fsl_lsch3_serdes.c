...
void serdes_init(uint32_t sd, uint32_t sd_addr, uint32_t rcwsr, uint32_t sd_prctl_mask, 
 uint32_t sd_prctl_shift, uint8_t serdes_prtcl_map[SERDES_PRCTL_COUNT]) 
{ 
 struct ccsr_gur __iomem *gur = (void *)(CONFIG_SYS_FSL_GUTS_ADDR); 
 uint32_t cfg; 
 int lane; 
 ...
 wriop_init_dpmac(sd, xfi_dpmac[lane_prtcl], (int)lane_prtcl); 
 
 if (lane_prtcl >= SGMII1 && lane_prtcl <= SGMII16) 
 wriop_init_dpmac(sd, sgmii_dpmac[lane_prtcl], (int)lane_prtcl); 
 break; 
 } 
#endif 
 } 
