...
static int sec_firmware_load_image(const void *sec_firmware_img)
{
    ...
    /*
     * The Exception Level must be EL3 to load and initialize
     * the SEC Firmware.
     */
    if (current_el() != 3) {
        ret = -EACCES;
...
int sec_firmware_get_random(uint8_t *rand, int bytes)
{
    ...
    if (!bytes || bytes > 8) {
        printf("Max Random bytes generation supported is 8\n");
        return -1;
    }
...
/*
 * fdt_fix_kaslr - Add kaslr-seed node in Device tree
 * @fdt:        Device tree
