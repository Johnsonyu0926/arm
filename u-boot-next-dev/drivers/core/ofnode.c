...
int ofnode_read_u32(ofnode node, const char *propname, u32 *outp)
{
	...
	if (ofnode_is_np(node)) {
		...
		cell = fdt_getprop(gd->fdt_blob, ofnode_to_offset(node), propname, &len);
...
const char *ofnode_read_string(ofnode node, const char *propname)
{
	...
	if (ofnode_is_np(node)) {
		struct property *prop = of_find_property(ofnode_to_np(node), propname, NULL);
		...
		str = fdt_getprop(gd->fdt_blob, ofnode_to_offset(node), propname, &len);
	}
	if (!str) {
		debug("<not found>\n");
		return NULL;
...
ofnode ofnode_find_subnode(ofnode node, const char *subnode_name)
{
	...
	if (ofnode_is_np(node)) {
		...
		int ooffset = fdt_subnode_offset(gd->fdt_blob, ofnode_to_offset(node), subnode_name);
...
int ofnode_read_u32_array(ofnode node, const char *propname, u32 *out_values, size_t sz)
{
	...
	if (ofnode_is_np(node)) {
		return of_read_u32_array(ofnode_to_np(node), propname, out_values, sz);
	} else {
		return fdtdec_get_int_array(gd->fdt_blob, ofnode_to_offset(node), propname, out_values, sz);
	}
}

int ofnode_write_u32_array(ofnode node, const char *propname, u32 *values, size_t sz)
{
	assert(ofnode_valid(node));
	debug("%s: %s: ", __func__, propname);

	if (ofnode_is_np(node)) {
		return of_write_u32_array(ofnode_to_np(node), propname, values, sz);
	} else {
		return fdt_setprop((void *)gd->fdt_blob, ofnode_to_offset(node), propname, values, sz);
	}
}
...
ofnode ofnode_first_subnode(ofnode node)
{
	...
	return offset_to_ofnode(fdt_first_subnode(gd->fdt_blob, ofnode_to_offset(node)));
}
...
ofnode ofnode_next_subnode(ofnode node)
{
	...
	return offset_to_ofnode(fdt_next_subnode(gd->fdt_blob, ofnode_to_offset(node)));
}
...
ofnode ofnode_get_parent(ofnode node)
{
	...
	if (ofnode_is_np(node))
		...
		parent.of_offset = fdt_parent_offset(gd->fdt_blob, ofnode_to_offset(node));
...
int ofnode_read_size(ofnode node, const char *propname)
{
	...
	if (ofnode_is_np(node)) {
		...
		if (fdt_getprop(gd->fdt_blob, ofnode_to_offset(node), propname, &len))
			return len;
	}
...
fdt_addr_t ofnode_get_addr_index(ofnode node, int index)
{
	if (ofnode_is_np(node)) {
		...
		prop_val = of_get_address(ofnode_to_np(node), index, &size, &flags);
		if (!prop_val)
			return FDT_ADDR_T_NONE;
		na = of_n_addr_cells(ofnode_to_np(node));
		return of_read_number(prop_val, na);
	} else {
		return fdt_get_base_address(gd->fdt_blob, ofnode_to_offset(node));
	}
...
{
	if (ofnode_is_np(node)) {
		return of_property_match_string(ofnode_to_np(node), property, string);
	} else {
		int ret;

		ret = fdt_stringlist_search(gd->fdt_blob, ofnode_to_offset(node), property, string);
...
int ofnode_read_string_index(ofnode node, const char *property, int index, const char **outp)
{
	if (ofnode_is_np(node)) {
		return of_property_read_string_index(ofnode_to_np(node), property, index, outp);
	} else {
		int len;

		*outp = fdt_stringlist_get(gd->fdt_blob, ofnode_to_offset(node), property, index, &len);
		if (len < 0)
			return -EINVAL;
		return 0;
	}
...
int ofnode_read_string_count(ofnode node, const char *property)
{
	if (ofnode_is_np(node)) {
		...
		return fdt_stringlist_count(gd->fdt_blob, ofnode_to_offset(node), property);
	}
}

static void ofnode_from_fdtdec_phandle_args(struct fdtdec_phandle_args *in, struct ofnode_phandle_args *out)
{
...
static void ofnode_from_of_phandle_args(struct of_phandle_args *in, struct ofnode_phandle_args *out)
{
	assert(OF_MAX_PHANDLE_ARGS == MAX_PHANDLE_ARGS);
...
int ofnode_parse_phandle_with_args(ofnode node, const char *list_name, const char *cells_name, int cell_count, int index, struct ofnode_phandle_args *out_args)
{
	if (ofnode_is_np(node)) {
		...
		ret = of_parse_phandle_with_args(ofnode_to_np(node), list_name, cells_name, index, &args);
		if (ret)
			return ret;
		...
		ret = fdtdec_parse_phandle_with_args(gd->fdt_blob, ofnode_to_offset(node), list_name, cells_name, cell_count, index, &args);
		if (ret)
			return ret;
...
int ofnode_count_phandle_with_args(ofnode node, const char *list_name, const char *cells_name)
{
	if (ofnode_is_np(node))
		return of_count_phandle_with_args(ofnode_to_np(node), list_name, cells_name);
	else
		return fdtdec_parse_phandle_with_args(gd->fdt_blob, ofnode_to_offset(node), list_name, cells_name, 0, -1, NULL);
}
...
static int decode_timing_property(ofnode node, const char *name, struct timing_entry *result)
{
	int length, ret = 0;
...
int ofnode_decode_display_timing(ofnode parent, int index, struct display_timing *dt)
{
	int i;
	ofnode timings, node;
	u32 val = 0;
...
const void *ofnode_get_property(ofnode node, const char *propname, int *lenp)
{
	if (ofnode_is_np(node))
		...
		return fdt_getprop(gd->fdt_blob, ofnode_to_offset(node), propname, lenp);
}
...
int ofnode_get_first_property(ofnode node, struct ofprop *prop)
{
	...
	if (ofnode_is_np(node)) {
		...
		prop->offset = fdt_first_property_offset(gd->fdt_blob, ofnode_to_offset(prop->node));
		if (prop->offset < 0)
			return prop->offset;
	}
...
int ofnode_get_next_property(struct ofprop *prop)
{
	if (ofnode_is_np(prop->node)) {
		prop->prop = of_get_next_property(ofnode_to_np(prop->node), prop->prop);
		if (!prop->prop)
			return -FDT_ERR_NOTFOUND;
	} else {
		prop->offset = fdt_next_property_offset(gd->fdt_blob, prop->offset);
		if (prop->offset < 0)
			return prop->offset;
	}
...
const void *ofnode_get_property_by_prop(const struct ofprop *prop, const char **propname, int *lenp)
{
	if (ofnode_is_np(prop->node))
		return of_get_property_by_prop(ofnode_to_np(prop->node), prop->prop, propname, lenp);
	else
		return fdt_getprop_by_offset(gd->fdt_blob, prop->offset, propname, lenp);
}
...
bool ofnode_is_available(ofnode node)
{
	if (ofnode_is_np(node))
		...
		return fdtdec_get_is_enabled(gd->fdt_blob, ofnode_to_offset(node));
}

fdt_addr_t ofnode_get_addr_size(ofnode node, const char *property, fdt_size_t *sizep)
{
	if (ofnode_is_np(node)) {
		int na, ns;
		int psize;
		...
		*sizep = of_read_number(prop + na, ns);
		...
		return fdtdec_get_addr_size(gd->fdt_blob, ofnode_to_offset(node), property, sizep);
	}
}
