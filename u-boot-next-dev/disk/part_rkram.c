...
static int rkram_part_init(struct blk_desc *dev_desc, struct list_head *parts_head)
{
    ...
    for (i = 0; i < t->u.ram_part.count; i++) {
        ...
        memcpy((char *)&part->name, (char *)&t->u.ram_part.part[i].name, len);
        ...
        debug("%s: name=%s, start=0x%lx, size=0x%lx, blksz=0x%lx\n",
              __func__, part->name, part->start, part->size, dev_desc->blksz);
    }
...
static void part_print_rkram_part(struct blk_desc *dev_desc)
{
    ...
    list_for_each(node, &parts_head) {
        ...
        printf("%3d\t0x%08lx\t0x%08lx\t%s\n", (i++ + 1), p->start, p->size, p->name);
    }
}

static int part_get_info_rkram_part(struct blk_desc *dev_desc, int idx, disk_partition_t *info)
{
