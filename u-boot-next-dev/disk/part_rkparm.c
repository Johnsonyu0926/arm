...
static int rkparm_param_parse(char *param, struct list_head *parts_head, struct blk_desc *dev_desc)
{
...
static int rkparm_init_param(struct blk_desc *dev_desc, struct list_head *parts_head)
{
    char *parts_list;
...
static int part_get_info_rkparm(struct blk_desc *dev_desc, int idx, disk_partition_t *info)
{
    struct list_head *node;
    struct rkparm_part *p = NULL;
    int part_num = 0;
