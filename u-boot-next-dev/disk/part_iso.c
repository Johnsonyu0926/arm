...
unsigned long iso_dread(struct blk_desc *block_dev, lbaint_t start, lbaint_t blkcnt, void *buffer)
{
	unsigned long ret;
...
int part_get_info_iso_verb(struct blk_desc *dev_desc, int part_num, disk_partition_t *info, int verb)
{
	...
	while (offset < 2048) {
		...
		if ((pide->boot_ind == 0x88) || (pide->boot_ind == 0x00)) { /* Header Id for default Sections Entries */
...
found:
	...
	case 0x01:
		info->size = 2400 >> 2;
		break; /* 1.2MByte Floppy */
	case 0x02:
		info->size = 2880 >> 2;
		break; /* 1.44MByte Floppy */
	case 0x03:
		info->size = 5760 >> 2;
		break; /* 2.88MByte Floppy */
	case 0x04:
		info->size = 2880 >> 2;
		break; /* dummy (HD Emulation) */
	default:
		info->size = 0;
		break;
	}
...
static int part_get_info_iso(struct blk_desc *dev_desc, int part_num, disk_partition_t *info)
{
