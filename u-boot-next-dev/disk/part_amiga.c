...
{
    struct amiga_part_geometry *g = (struct amiga_part_geometry *)&(p->environment);
...
struct rigid_disk_block *get_rdisk(struct blk_desc *dev_desc)
{
    int i;
    int limit;
    char *s = env_get("amiga_scanlimit");

    limit = s ? simple_strtoul(s, NULL, 10) : AMIGA_BLOCK_LIMIT;
 ...
 */
static int part_get_info_amiga(struct blk_desc *dev_desc, int part, disk_partition_t *info)
{
...
static void part_print_amiga(struct blk_desc *dev_desc)
{
    ...
    printf("Summary:  DiskBlockSize: %d\n"
           ...
           "          Sectors/Track: %d\n"
           "          Heads        : %d\n\n",
           rdb->block_bytes, rdb->cylinders, rdb->sectors, rdb->heads);
