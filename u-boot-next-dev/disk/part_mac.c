...
/* stdlib.h causes some compatibility problems; should fix these! -- wd */
...
static int part_mac_read_ddb(struct blk_desc *dev_desc, mac_driver_desc_t *ddb_p);
static int part_mac_read_pdb(struct blk_desc *dev_desc, int part, mac_partition_t *pdb_p);
...
static int part_test_mac(struct blk_desc *dev_desc)
{
    ...
    if (part_mac_read_ddb(dev_desc, ddesc)) {
        /* error reading Driver Descriptor Block, or no valid Signature */
        return -1;
    }
...
static int part_mac_read_pdb(struct blk_desc *dev_desc, int part, mac_partition_t *pdb_p)
{
    ...
    for (;;) {
        /* We must always read the descriptor block for partition 1 first since this is the only way to know how many partitions we have. */
...
static int part_get_info_mac(struct blk_desc *dev_desc, int part, disk_partition_t *info)
{
    ALLOC_CACHE_ALIGN_BUFFER(mac_driver_desc_t, ddesc, 1);
    ALLOC_CACHE_ALIGN_BUFFER(mac_partition_t, mpart, 1);
