...
{
    shell_print(sh, "z_shared_sw_isr_table[%d][%d]\n", IRQ_TABLE_SIZE, CONFIG_SHARED_IRQ_MAX_NUM_CLIENTS);
    for (int idx = 0; idx < IRQ_TABLE_SIZE; idx++) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(isr_table_cmds,
    SHELL_CMD_ARG(sw_isr_table, NULL, "Dump _sw_isr_table.\nUsage: isr_table sw_isr_table", cmd_sw_isr_table, 1, 0),
#ifdef CONFIG_SHARED_INTERRUPTS
    SHELL_CMD_ARG(shared_sw_isr_table, NULL, "Dump z_shared_sw_isr_table.\nUsage: isr_table shared_sw_isr_table", cmd_shared_sw_isr_table, 1, 0),
#endif
    SHELL_SUBCMD_SET_END
);

SHELL_CMD_ARG_REGISTER(isr_table, &isr_table_cmds, "ISR tables shell command", NULL, 0, 0);
// By GST @Date